# Questões de SQL — Sistema de Gestão de Consultas MedConsulta

> Banco: **MedConsulta** (SQL Server / T-SQL)
>
> São 90 questões organizadas em 10 blocos de 9. As questões possuem um nível integrado de complexidade, cobrando a união de JOINs, subconsultas, agregações e filtros em uma única instrução.
>
> Conteúdo SQL permitido nas queries: consultas simples, JOINs, CASE WHEN, subqueries (em SELECT, FROM e WHERE), funções de agregação, HAVING e CTEs (`WITH`). **Não devem ser utilizados** *window functions* (`OVER`) nem `STRING_AGG`.

---

## Bloco 1 (Q01–Q09)

01. Identifique as consultas agendadas pelo Dr. Roberto Santos para pacientes residentes no estado de Sao Paulo (SP), exibindo o nome do paciente, o documento do paciente, a data/hora do agendamento e o nome do médico.
