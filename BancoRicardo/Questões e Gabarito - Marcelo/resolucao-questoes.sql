...
SELECT	us.Nome as Usuario,
		us.Email as Email,
		lo.DataLogon as DataDeTentativa
...
SELECT	DATEPART(YEAR, lo.DataLogon) as Ano,
		DATEPART(MONTH, lo.DataLogon) as Mes,
		COUNT(lo.IdUsuario) as QuantidadeLogons
	FROM Logon AS lo WITH(NOLOCK)
	GROUP BY DATEPART(YEAR, lo.DataLogon), DATEPART(MONTH, lo.DataLogon);
...
SELECT	us.Nome as Usuario,
		COUNT(lo.Id) as QuantiadeLogons
	FROM Usuario AS us WITH(NOLOCK)
		LEFT JOIN Logon AS lo WITH(NOLOCK)
			ON lo.IdUsuario = us.Id	
		CROSS JOIN (		            
		            SELECT  COUNT(lo2.Id) * 1.0 / COUNT(DISTINCT us2.Id) as MediaLogons
			            FROM Logon AS lo2 WITH(NOLOCK)
						    RIGHT JOIN Usuario AS us2 WITH(NOLOCK)
								ON us2.Id = lo2.IdUsuario
				   ) AS Media
	GROUP BY us.Nome, us.Id, Media.MediaLogons
	HAVING COUNT(lo.Id) > Media.MediaLogons; 
